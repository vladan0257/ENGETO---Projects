...
def login(name, password):
    ### This function iterates through name-password pairs in the bellow dictionary and controls them 
    ### againts user-inputs. If a match is triggered, the program stops there and gives the user free pass.
    registeredUsers = {'bob': '123', 'ann': 'pass123', 'mike': 'password123', 'liz': 'pass123'}
    match = 0
    for dictName, dictPassword in registeredUsers.items():
        if name == dictName and password == dictPassword:
            match = match + 1
        if match:
            return True
    if not match:
